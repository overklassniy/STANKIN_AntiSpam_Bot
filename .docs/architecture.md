...
sequenceDiagram
    ...
    MOD->>MOD: Проверка системного ID

    alt Автор не админ и не системный
        MOD->>DB: Проверка белого списка
        alt В белом списке
            MOD->>NOTIFY: Логирование в топик (если включено)
            MOD-->>DP: Пропустить
        else Не в белом списке
            MOD->>MOD: Логирование в топик (если включено, до анализа)
    ...
    else Автор админ или системный
        MOD-->>DP: Пропустить
    end
```
